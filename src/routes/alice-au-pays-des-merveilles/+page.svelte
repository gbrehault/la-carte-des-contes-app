...
<section bind:this={section} class="relative h-[100vh] bg-cover bg-center">
  <a href="/">
    <img src={Logo} alt="" class="fixed top-0 right-0 w-30 z-999" />
  </a>
  <div
    id="bloc-video"
    class="sticky flex-col top-0 h-screen flex items-center justify-center"
  >
    <h1 class="text-5xl font-bold text-alice-light font-title">
      ALICE AU PAYS DES MERVEILLES
    </h1>
    <h4 class="text-3xl font-regular text-dark font-sb">Prochainement</h4>
    <!-- <video
      bind:this={video}
...
<section class="flex flex-col items-center justify-center pt-30">
  ...
  <div class="flex items-center jsutfify-center my-20 px-4">
    <h3 class="font-sb text-3xl max-w-4xl text-center z-999">
      Dans le pays des merveilles, tous les monstres ne montrent pas leurs
      crocs. Certains portent un sourire, d’autres murmurent des mots doux pour
      mieux tromper les cœurs naïfs. Alice, curieuse et courageuse, l’a appris
      au détour d’un rêve : le danger ne se cache pas toujours dans l’ombre,
      mais parfois dans une voix douce, un regard trop gentil. Alors, quand
      quelque chose lui semblait étrange, elle écoutait son instinct — cette
      petite voix qui chuchote la vérité, même quand tout paraît merveilleux. Et
      si la peur venait frapper à sa porte, Alice savait qu’elle n’était pas
      seule. Parler, c’est ouvrir une lumière. Un adulte de confiance,
      disait-elle, saura entendre… et protéger.
    </h3>
  </div>
  <div
  ...
  >
    ...
    >
      <path
        ...
        class="fill-alice-light stroke-alice-light transition-all duration-300 group-hover:fill-alice-dark group-hover:stroke-alice-dark"
      />
    </svg>
    <a
      href="/"
      class="font-title font-bold text-3xl pt-2 text-alice-light transition-colors duration-300 group-hover:text-alice-dark"
    ...
    >
      <path
        ...
        class="fill-alice-light transition-all duration-300 group-hover:fill-alice-dark"
      />
      <path
  ...
  <h2 class="font-bold text-alice-light font-title text-5xl text-center mt-30">
